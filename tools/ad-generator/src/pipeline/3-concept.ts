...
// Simplified tip taxonomy. Old types (form-check, antrenman, beslenme, basari) all converge
// on similar Problem→Solution structures already covered by `klasik` or `zevo-template`.
// Keeping only 3 modes: shared template / free creative / minimalist no-UI.
// Old type strings still PARSE (back-compat) and map to the closest current mode.
export type ConceptType = "zevo-template" | "klasik" | "motivasyon";
export type ConceptTypeLegacy = ConceptType | "form-check" | "antrenman" | "beslenme" | "basari" | "kurucu";
function normalizeType(t: ConceptTypeLegacy | string | undefined): ConceptType {
  if (t === "zevo-template" || t === "klasik" || t === "motivasyon") return t;
  if (t === "motivasyon") return "motivasyon";
  // form-check, antrenman, beslenme, basari, kurucu, anything else → klasik (most flexible)
  return "klasik";
}
